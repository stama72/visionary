...
2回目(再凍結後): **止まった 0 / 決めた 0**(初回実装・レビュー修正・実測の転記の3回とも 0 / 0)。
...
| 巡 | 守備範囲 | 象限I | 象限II | 疑い |
| -- | -------- | ----- | ------ | ---- |
| 1 | 1巡目 | 1(I-a。直した) | 0 | 1(M-1 の実測が #237 前の世代。mutator の測定に足して解消) |
| 2 | 象限I と疑い | 0 | — | 0 |
...
11件(仕様 M1〜M9 + レビュー追加 M10・M-1)。期待と食い違い 0、ビルド不能 0、場所が特定できない 0。M8 は必需・入力の両ループに当てた(テストの失敗は必需側 −40 で出た。入力側単独の変異は測っていない)。
