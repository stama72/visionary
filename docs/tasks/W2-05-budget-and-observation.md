...
**R1-7 の「この実装ミスで落ちる」欄を狭めた**(「最小/最大」→「**最小**」)。`RankCoefficientPermille` は Master(1000‰)> Journeyman(600‰)> Apprentice(200‰)の単調順であり、**M0 の世帯主は全員 Master なので、「構成員の最大を採る」実装と「世帯主を採る」実装は M0 では観測上まったく同じ結果を出す。** 差が出るのは [GDD10](../03-gdd/10-employment.md) の雇用で階層の違う世帯主が現れてからである。**テストを増やす価値は無いので、主張のほうを狭い側に倒した** — 広い主張を残すと「この誤りは表が押さえてある」と読ませ、踏んでも気付けなくなる([process/03](../process/03-corrections.md))。
...
**2件とも R1 別表と同じ類型である** — `BuyerDemand.Build` の**呼び出し側の配線**のうち、テストが一度も踏まない経路。R1 別表は3つ(嗜好の母数 / `selfHouseholdId` / フォールバック比率)を埋め、ここで**耐久の相場基準**と**派生需要の按分枝**を足す。**指摘の種類を捉えることと、その種類の全インスタンスを列挙することは別の作業である。**

> **この類型は、ここでも列挙し切れていない。** 上の文は当初「5経路のうち3つ」と全体の件数を断定していたが、**4巡目が網羅したところ同じ類型が10件残っていた**(A〜J)。件数を断定する書き方そのものが、**列挙が完了したという保証**として読まれる([process/03](../process/03-corrections.md))。**この別表は「塞いだ経路」を数えているのであって、「残りが無い」とは一切主張しない。** 残りの一覧と、直さずに issue へ落とした理由は[引き継ぎメモ](W2-05-budget-and-observation.handoff.md)が持つ。**#37 は「W2-05 側でこの配線が守られている」を前提にしてはならない。**
