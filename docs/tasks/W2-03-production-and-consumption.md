...
| 31 | `ProductionDividesAndMultipliesByInputQuantity` | **入力A(数量2・在庫4)と入力B(数量3・在庫15)を持つレシピ**で生産能力3 → 実行 **2回**。入力Aは **0**、入力Bは **9** が残る | **`FloorDiv(在庫, 必要数量)` を書かず在庫の個数で比べる**(実行3回になり入力Aが **−2** に落ちる)。**`-= 必要数量 × 実行回数` の掛け算を落とす**(入力Aに 2、入力Bに 13 が残る) | **核心** |
