...
> **「売り手ごとに読んでは書く」が結果を変える経路は、実は1つしかない。** 相場基準の材料は(a)世帯主の観測(前日まで。この間 `Knowledge` は動かない)と(b)`Market[(品目, 自世帯)]` = **自分の**前日価格だけで、他の売り手の当日価格は入らない。**壊れるのは「先に `Market.Clear()` してしまう」形であり、そのとき全売り手が自分の前日価格を失う。** テスト #22 がこれを見る。
...
- **`BuildM0` の値に依存したテストを書かない**(#28 が値を動かす)。テスト専用の `WorldDefinition` を組み立てること。**#21 だけは例外的にパイプラインを M0 の定義で回すが、期待値は定義から計算して比較する**(数値をハードコードしない)
...
**上の表は実装に渡した時点の指示であって最終形ではない。** レビュー(フェーズ2)で判別力の穴が見つかり、後から足したものをここに分けて置く。

| #  | テスト | 検証内容 | この実装ミスで落ちる | 核心 |
| -- | ------ | -------- | -------------------- | ---- |
| R1 | `MarketReferenceExcludesTheHouseholdsOwnObservationEvenWhenHeadNpcIdDiffers` | `HeadNpcId = 2` ≠ 世帯 Id = 0 の世界で、世帯主の `Knowledge` に `SellerId = 0`(自世帯)の観測だけを置く → 相場基準が立たず**原価下限のまま** | `TryMarketReference` の `selfHouseholdId` に `household.HeadNpcId` を渡す(自分の売り注文が相場基準に入り、無関係な他世帯の観測が除外される)。**表 #16 は純関数側しか見ておらず、#26 は `Knowledge` の添字しか固定していない** | **核心** |

- **#16(`MarketReferenceExcludesOwnObservations`)との違いは呼び出し側を通すことである。** #16 は `OfferPrice` を直接呼ぶので `selfHouseholdId` に何を渡すかは `TradeSystem` の側で自由に間違えられる。[TDD01 §3.2](../04-tdd/01-sim-core-and-m0.md) が NpcId と世帯 Id の取り違えを「**型で防ぐことはできない。検出は回帰テストに頼る**」と書いた経路がここである
- **表 #21 に `Assert.NotEmpty(world.Market)` を足した。** 元の形は「販売在庫0の世帯はエントリが無いことを確認する」ループだったので、**全世帯の在庫が0になると値付けを1度も検証せずに緑になる**。いまは `initialWorkshopInputDays: 5` が空振りを防いでいるが、この値は #28 が動かす
