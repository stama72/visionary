...
### `SellerReferenceIsTakenBeforeTheSellableStockGate` の初期条件の組み直し(2026-09-21 の訂正。フェーズ2)

**上の2つの逃げ道が、このテストにだけ構造的に効かない。** フェーズ2 の報告:世帯は「段6の `IsWithinReach` を常に真にする」ために中心区画へ固定されており、**中心から遠ざけられない**。そして距離0では `ErrandPlanner.EstimateWindowPrice` が「今日の知覚」の段に落ちるので、**高値の記憶を持たせても窓口を外せない**。

**保たれるべき判別力は順序である** — 「売り手の相場基準は、販売可能在庫のゲートより**先に**取られる」。「相場基準51・床100・数量1」という具体値ではない。したがって:

- **初期条件(在庫・所持金・品目定義・相場の材料)を組み直し、期待値を再導出してよい。** 上の「期待値の置き直し」の規則の適用であって、新しい設計判断ではない。組み直した行には変わった理由を doc コメントで1行残す
- **ただし組み直した世界で「相場基準を取る段とゲートの段を入れ替えても緑になる」なら、そのテストは死んでいる。** その場合は緑にせず止まって報告する(判別力の吸収そのものである)
- **窓口を外すために `IsPrimaryItem` の割り当て(fixture の品目定義)を動かすのも可。** ただし動かした品目が、そのテストが見ている輸出・相場の経路に関わらないことを確かめてから動かす
