...
if 工房在庫[Item.Tools] == 0:  household.ToolWear = 0      ← 無条件。実行回数 0 の日も通る(GDD02a §3.1)
```

**実行回数 0 の日に早期 return しない(3巡目 象限I の訂正)。** 本節の前版は `household.ProductionRuns` を書いた直後に `if 実行回数 <= 0: 在庫も摩耗も動かさない。次の世帯へ` を置いていた。**実行回数 0 なら在庫の増減も摩耗の加算もすべて 0 の掛け算になるので、この早期 return は摩耗の破棄を飛ばすためだけに効いていた。** [GDD02a §3.1](../03-gdd/02a-production.md) は加算・消費ループ・破棄の3つを同じ字下げで並べ、ブロック全体に「毎日、§1 の生産のあとに」を掛けている — **破棄は生産量 0 の日も含めて毎日走る。**

**破棄は `worn > 0` の外である(1巡目 象限I-b の訂正)。** [GDD02a §3.1](../03-gdd/02a-production.md) は「工具在庫が 0 なら累積摩耗を 0 に戻す」をループと同じ字下げ — つまり日次の無条件の手順 — として書いており、本節の初版はこれを `worn > 0` の内側に入れ子にしていた。**W2-03 までは無害だった** — 工具 0 個なら設備係数 0‰ で生産が止まり、工具 0 個の日に摩耗が積まれる経路が存在しなかったので、入れ子の有無で挙動が一致していた。**本タスクが「工具なしでも 500‰ で生産を続ける」に変えたことで、その経路が初めて開く。** 入れ子のままだと、工具を失った世帯が工具 0 個のまま摩耗を積み続け、次に買った工具から差し引かれる(GDD02a §3.1 が「次に買った工具が買ったその日に壊れうる」として名指しで禁じた挙動)。副作用として `BuyerDemand` の耐久の予想在庫 `工具在庫 × 耐久値 − ToolWear` が負になる。

**守るのは「工具在庫 0 なら `ToolWear` も 0」という日末の不変量であり、それを他システムの都合に預けない。** 工具が工房在庫から出ていく経路は摩耗のほかに**売却**がある(`TradeSystem` の売却可能在庫は工房在庫の全量で、工具を留保しない)。工具を出力に持つ鍛冶は、`ToolWear > 0` のまま工具を売り切りうる。**その翌日の鍛冶は `floor(floor(1300 × 500 ÷ 1000) ÷ 1000) = 0` 回**(本節の具体例そのもの)なので、早期 return があると破棄が一度も走らず、古い `ToolWear` が残り続ける。**「売却側が最後の1個を留保するから大丈夫」には倒さない** — 不変量は `ToolWear` を持つ側が構造で守る。留保の有無は #97 / #38 の所掌であり、そちらが変わっても本タスクの不変量が崩れてはならない。
...
- **労働損失 100‰(徒弟が往復 4 時間)のパン屋(216‰)**: `(1300 − 100) ÷ 216 = 5.55 → 5 回`(損失なしなら 6 回)。**損失 1300‰ 以上なら 0 回**で、在庫も累積摩耗の加算も動かず `ProductionRuns = 0`。**ただし摩耗の破棄だけは走る**(上の擬似コード末尾)
- **工具が無い水車小屋番(185‰)**: `floor(1300 × 500 ÷ 1000) = 650`、`650 ÷ 185 = 3.51 → 3 回`(工具ありなら 7 回)。**0 回ではない** — 旧仕様の「工具が無ければ停止」は消えた
- **工具が無い鍛冶(1000‰)**: `650 ÷ 1000 → 0 回`。これが M0 で唯一「労働力不足」(GDD02b §8)が立つ組み合わせだが、Need を立てるのは #40。**同時に、実行回数 0 と工具在庫 0 が同じ日に重なる唯一の職業でもある** — 破棄を早期 return の内側に置くと、ここだけが黙って壊れる
...
| 11 | `EquipmentWithoutToolsZeroStopsProduction` | 同上で工具なし係数 0‰ → 0 回、在庫も**累積摩耗の加算**も動かない(破棄は別。別表 #30 が見る) | `max(…, 1)` などで底上げする |  |
...
**上の表は実装に渡した時点の指示であって最終形ではない。** ここはレビューで足したぶんで、上の表と同じ拘束力を持つ。

| #  | テスト | 検証内容 | この実装ミスで落ちる | 核心 |
| -- | ------ | -------- | -------------------- | ---- |
| 29 | `ToolWearIsDroppedOnEveryDayWithoutTools` | 表 #9 の続き。工具が尽きた**翌日以降も**、工具 0 個で生産が続くあいだ日末の `ToolWear` が **0 のまま**であること。`N = 1`・所要労働 400‰ で工具を失ったあと、工具なし係数で 1 回/日 実行した翌日に **`ToolWear == 0`**(耐久値 1000 に届かないので `worn == 0` の経路を通る) | 破棄を `worn > 0` の内側に入れ子にする(1巡目 象限I-b。`ToolWear == 400` が残り、次に買った工具から差し引かれる)。`worn == 0` の枝で早期 return する | **核心** |
| 30 | `ToolWearIsDroppedOnDaysWithZeroProduction` | **実行回数 0 の日も破棄が走る**こと。`N = 2`(耐久値 2000)・所要労働 400‰・労働力 1300‰・**工具なし係数 0‰** の定義で、工具在庫 0・`ToolWear = 1500` を**直接置いて** 1 日進める → `ProductionRuns == 0` かつ日末の **`ToolWear == 0`**、予想在庫 `0 × 2000 − 0` が非負。続けて工具を 1 個与えて 1 日進める → 3 回実行して **`ToolWear == 1200`(< 2000)で `工具在庫 == 1` のまま** | 破棄を `if 実行回数 <= 0` の早期 return の内側に置く(3巡目 象限I)。1日目に `ToolWear == 1500` が残り、2日目が `1500 + 1200 = 2700 ≥ 2000` で **買ったその日に工具が消費され `工具在庫 == 0`** になる | **核心** |

- **番号は本タスクの通し番号である**(W2-03 の #30 とは別)
- **#29 は表 #9 と同じ工具・労働量の設定を使い、日数だけ伸ばす。** 別のフィクスチャを立てると、#9 が押さえている「尽きた日の破棄」と #29 が押さえる「尽きた翌日以降の破棄」が別の前提の上に乗り、片方を壊してももう片方が緑のまま残る
- **`ToolWear` が 0 であることに加えて、`BuyerDemand` の耐久の予想在庫 `工具在庫 × ToolDurabilityPerUnit − ToolWear` が非負であることも確かめる**(#29・#30 とも)。これが元の指摘で挙がった実害の経路である
- **#29 の前版は「その翌日に工具を 1 個与えて、新品が前日までの摩耗を引き継がないことを確かめる」という 3 日目を持っていたが、判別力が無かったので #30 へ移した**(3巡目 象限I)。#29 の 2 日目が `ToolWear` を 0 に落とした後では、3 日目の開始状態は 1 日目と完全に同一(工具 1 個・`ToolWear` 0)であり、**引き継ぎの有無にかかわらず同じ結果に収束する。** #30 は引き継ぎが起きる状態を**直接置く**ことでそこを判別する(状態を直接置いて読み手を確かめるのは表 #16・#17 と同じ手である)
- **#30 が実行回数 0 を作るのに使うのは「工具なし係数 0‰」である。** 入力切れで 0 回にすると、破棄が走らない実装でも入力の枯渇が先に目立ち、何が壊れているのかが読めなくなる
