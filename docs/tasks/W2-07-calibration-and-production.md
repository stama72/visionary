...
if 工房在庫[Item.Tools] == 0:  household.ToolWear = 0      ← worn の外。無条件(GDD02a §3.1)
```
...
| #  | テスト | 検証内容 | この実装ミスで落ちる | 核心 |
| -- | ------ | -------- | -------------------- | ---- |
| 29 | `ToolWearIsDroppedOnEveryDayWithoutTools` | 表 #9 の続き。工具が尽きた**翌日以降も**、工具 0 個で生産が続くあいだ日末の `ToolWear` が **0 のまま**であること。`N = 1`・所要労働 400‰ で工具を失ったあと、工具なし係数で 1 回/日 実行した翌日に **`ToolWear == 0`**(耐久値 1000 に届かないので `worn == 0` の経路を通る)。さらにその翌日に工具を 1 個与えて 1 日進め、**新品が前日までの摩耗を引き継がない**こと | 破棄を `worn > 0` の内側に入れ子にする(1巡目 象限I-b。`ToolWear == 400` が残り、次に買った工具から差し引かれる)。`worn == 0` の枝で早期 return する | **核心** |

- **#29 は表 #9 と同じ工具・労働量の設定を使い、日数だけ伸ばす。** 別のフィクスチャを立てると、#9 が押さえている「尽きた日の破棄」と #29 が押さえる「尽きた翌日以降の破棄」が別の前提の上に乗り、片方を壊してももう片方が緑のまま残る
- **`ToolWear` が 0 であることに加えて、`BuyerDemand` の耐久の予想在庫 `工具在庫 × ToolDurabilityPerUnit − ToolWear` が非負であることも確かめる。** これが元の指摘で挙がった実害の経路である
