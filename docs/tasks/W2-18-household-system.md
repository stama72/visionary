...
| # | テスト | 検証内容 | この実装ミスで落ちる |
| - | ------ | -------- | -------------------- |
| 14 | `SelfOccupationIsExcludedEvenWhenItHasTheFewestCarriers` | **自職業の担い手が全候補より真に少ない**世界で3条件が揃ったとき、`argmin` が自職業を返さず、別の職業へ付け替わる。段A・段B の両方を通す | `TrySelectTarget` の「自職業を除く」を落とす(`argmin` が自職業を返し、`Occupation = target` が自己代入の no-op になる。ゲートは翌日も開いたままで、その世帯の④が**永久に発火しない**)。**#1〜#13 はこの変異で1つも落ちない**(1巡目の実測) |
...
| # | テスト | 守る契約 | 検証内容 |
| - | ------ | -------- | -------- |
| 21 | (`EveryOccupationKeepsAtLeastOneCarrierOverSixtyDays` に断定を足す) | **④が M0 の走行で実際に発火すること。** 裁定 D-A・D-B・D-C が固定値と日次下限を落とした根拠は「④が職業分布を動かす」の1点だが、**置き直した3本はどれも④が1度も発火しない世界でも緑になる**(D-A は 6/180/6 で下限を満たし、D-C は全職業2 のままで担い手 ≥ 1 を満たす)。④が到達不能になった日、残るのは**理由の消えた緩い検出器**である | 60日のうち**少なくとも1日、職業分布が day 0 と異なる**ことを断定する。日次の担い手数は D-C が既に数えているので、その走査に足すだけでよい。**最初に分布が動いた日をシードごとに測って remarks へ転記する** |

**あわせて `ScannedSelfSuppliableEntryCount` の doc コメントを直す**(D-A が置き換えた断定を指したまま残っている)。「6戸 × 30日で **180件になるはずである**(**固定値で** assert する)」「走査範囲が縮む変異をこの件数が捕まえる」はいずれも現行の断定では成り立たない。**判別力が 180 → 90 へ落ちたこと**と、**その結果として捕まえられなくなった変異**(自家供給3戸ぶんまで走査が縮む変異は、日次下限3・延べ下限90・day 0 の `SelfSuppliableHouseholdCount` のどれも通り抜ける)を**書く**。`R-3` / `R-4` の転記と「最終日の観測件数(6)を固定値で先に留める」の文も、現存しない断定を指しているので現行へ直す。**狭い側に倒すこと**([03-corrections](../process/03-corrections.md))
