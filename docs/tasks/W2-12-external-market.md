...
**では耐久の換算(`BuyerBudget.QuantityInUnits`)を何が守っているかは、机上で断定しない。** M-5 はもともと「測るだけ」であり、仕様は既に「**緑のままなら、それは耐久の換算が依然としてどこからも守られていないという実測である** — 直さずに #37 の後継として issue へ落とす」と決めている。**期待を書かず、`mutator` の実測に決めさせる。**

> **3巡目の訂正。** ここには当初「耐久の換算には現時点で検出器が無い」と書いていたが、**同じ変更で足した R-6 の前提表明がこれを反証していた。** R-6 は工具の `Purchase` 行を「数量 == 1」で押さえており、M-5 を当てると数量が 1 → 50 になってこの行が崩れる。**主題が「段1 の相場基準の位置」であるテストの前提行が、偶然の検出器になっている。** 「検出器が無い」は読者に確認をやめさせる記述であり、実測の前に書いてよいものではなかった([process/03](../process/03-corrections.md))。**R-6 の前提表明を緩めると、この偶然の守りが黙って消える** — R-6 の doc コメントにその旨を残す。
