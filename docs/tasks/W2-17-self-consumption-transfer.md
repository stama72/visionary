...
### 別表 — レビューで足したテスト(フェーズ2・2026-09-22)

**上の表は実装に渡した時点の指示であって最終形ではない。** レビューの巡で足したものをここに分けて置く。

| # | テスト | 足した理由(何巡目のどの指摘か) |
| - | ------ | -------------------------------- |
| 11 | `IsHoardedWhileEmptyMatchesTheHoardingDefinition` | **2巡目 I-a-1。** #7 の核心 `HoardedWhileEmptyEntries.Count == 0` は、M0 では式の帰結として 0 件になる(上の「#7 の doc コメントに書くこと」)。したがって**「経済が健全だから0件」と「走査が壊れていて0件」が区別できない。** 走査を「全観測(180件 = 自家供給6戸 × 出力品目1 × 30日)の記録」と「その上の絞り込み」に分け、**絞り込みの述語を静的ヘルパーへ切り出して**合成の観測値で直接突く。判定式の取り違えが、30日の走行を伴わずに落ちる |

**#7 に足した留め具(同じく2巡目 I-a-1 と3巡目 I-a-1)**: 空振り防止として **観測件数 == 180**(固定値)と、**最終日の観測を走行後の `world` の実際の在庫と突き合わせる** assert を、核心の手前に置いた。前者は走査が回らない / 範囲が縮む / 記録そのものが消える変異を、後者は**両項が同じ配列を読む取り違え**(述語が恒偽化し、全留め具を緑にしたまま検出器が死ぬ)を落とす。

**左項(世帯在庫が0だった観測)の到達可能性は assert しない。** **seed 42 では自家供給6戸すべてが30日間一度も世帯在庫0まで下がらない**(フェーズ2 実測、2026-09-22)。**走査の欠陥ではなく、そのシードでは検出対象の前提が成立していないことである。** 正であることを要求すると恒常的に赤くなる。
