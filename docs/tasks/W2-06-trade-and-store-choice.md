...
**#36 が作った `BuyerDemand` / `BuyerBudget` は、本タスクで初めて呼ばれる。** [#37 に引き継がれた配線10件(A〜J)](https://github.com/stama72/visionary/issues/37)は、**いまテストが一度も踏んでいない**。本タスクのパイプライン統合テストがそれを構造的に踏む(下記「統合」節)。**W2-05 側で配線が守られていることを前提にしてはならない。**
...
| 22 | `PreferenceAndDurableAreActuallyBought` | 30日回した後、**ビール(嗜好)の約定が1件以上**あり、**工具(耐久)の約定が1件以上**ある | #36 引き継ぎ表 **A**(耐久の母数を余剰資金にする → 運転資金で潰れた世帯が恒久に工具を買えない)、**C**(嗜好の目標在庫が0 → ビールが一度も買われない)、**D**(嗜好の予想在庫が工房在庫) | **核心** |
...
- [ ] 「落ちるべき条件」のテストが全て緑
- [ ] **「核心」印のうち #6・#7・#12・#17・#20・#26 に変異を当てて落ちることを確認し、当てた変異と結果を残した**
