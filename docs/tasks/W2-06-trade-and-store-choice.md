...
>
> **落ちる場所は日数で変わる。2巡目の実測で、1巡目に書いた機構の記述が誤っていたので訂正する。**
>
> | 世界 | 嗜好(ビール)が落ちる場所 | 資金を積むと |
> | ---- | -------------------------- | ------------ |
> | **1日だけ回す** | **手順2**(`TrySelect` が false)。初日は他の醸造家の売り注文の観測がまだ無く、**知っている店が0件** | **変わらない**(資金を1000倍にしても買えない) |
> | **2日以上回す** | 手順3(`store.UnitRealCost > line.Budget`) | 約580 で通過する |
>
> **資金の帯が重ならないのは2日目以降の話である。** 必需の基礎値は**流動資金**に比率を掛けるが([GDD02 §8.2.7](../03-gdd/02-economy.md) のフォールバック)、嗜好の基礎値は **`SurplusFunds = max(0, 流動資金 − 必要運転資金)`** に掛ける([GDD02 §8.2.1](../03-gdd/02-economy.md))。必要運転資金が大きいため、**嗜好が手順3 を通過できる水準(約580)では、必需の1日あたりの必要額(冬季でも約33)が無視できるほど小さい。** 逆に必需が資金不足に落ちるまで絞ると(50〜200)、嗜好は手順3 で落ちて手順8 に到達せず、資金を奪い合わない。
>
> 探索範囲(seed 1・世帯 Id 0): 資金 50〜1200 を粗く、300〜620 を5刻み、50〜300 を10刻み × 秋/冬 × 1・2・5日目 × 正順/逆順。**`UnaffordableNecessityCount`・嗜好の約定の有無・購入後の流動資金がいずれも1桁まで一致した。** 薪の冬の季節係数 2000‰ は必需側を約16 → 約33 に動かすだけで、約17倍の差を埋めない。
>
> **走査順そのものは実装が守っている**(段5 は `Lines` を並べ替えていない)。**守られていないのは「テストで守られている」という保証のほうである。** 検出器の置き場所(段5 を狭く踏む単体テストを起こすか、#39 が困窮の指標を実際に読むときに併せて立てるか)は本タスクのスコープ外なので **issue へ落とす**。**その担当者は上の表から出発すること** — 1日だけの世界では資金をいくら積んでも嗜好は買えないので、資金帯を探しても見つからない。
>
...
> **#28 は「空間の摩擦が消えたら落ちる」テストではない。当ててみた変異をどれも落とさなかった**(レビュー1・2巡目の実測。seed 1・30日)。
>
> | 当てた変異 | #28 | 赤になったテスト |
> | ---------- | --- | ---------------- |
> | 実質コストから移動費を落とす | **緑** | — |
> | 実質コストの argmin をやめ、提示価格で選ぶ(`realCost = price`) | **緑** | #10 のみ |
> | 候補集合と移動費を全買い手で共通化する(`isKnown` を常に真 + 距離を固定) | **緑** | #12・#15・#24 |
