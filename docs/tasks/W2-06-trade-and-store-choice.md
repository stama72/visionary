...
>
> **切り出しても、元の変異(段5 がヘルパーを通さず `line.TargetStock` をそのまま渡す)は依然として落ちない**(レビュー2巡目の実測。呼び出し側からヘルパー2つを外して**全317件が緑**)。**#6 が守るのはヘルパーの内側の分岐だけであり、呼び出し側がヘルパーを通ることは誰も守っていない。** W2 では耐久の約定が構造的に起きないので、呼び出し側の換算漏れは観測できる挙動を一切変えない — **この穴は W2 では原理的に塞げない。** #38 で耐久の約定が起きるようになって初めて踏める(下記「#38 へ」に引き取り条項を足した)。
...
| 25 | `NecessityIsSettledBeforePreference` | 必需だけが買える資金しか持たない世帯を作り(初期資金を絞った定義で回す)、**必需の約定が成立し、嗜好の約定が0件**である。**絞った世界は2日以上回すこと**(レビュー3巡目 I-b の訂正。**1日だと嗜好の行が手順4(在庫圧力が立たず購入量0)で落ちるため、嗜好側の assert が資金に依存しなくなる** — 資金を1000倍にしても緑のまま空振りする。**1日でも緑になるので、下の「日数は実装が緑にできる最小でよい」に従って縮めてはならない**)。**あわせて「資金さえあれば嗜好は買える」ことを対照で確かめる** — 同じ世帯の初期資金だけを増やした世界で**嗜好の約定が成立する**こと(レビュー1巡目 I-b の訂正。これが無いと嗜好が手順3(予算 < 実質コスト)で**順序と無関係に**落ち、並べ替え変異を素通りさせる) | `Lines` を並べ替える / 用途でソートし直す(**「資金不足で必需品が買えなかった」が買い物の順序の関数になり、困窮の指標として読めなくなる**。GDD02 §6.2.1)。**ただし W2 ではこの変異は落ちない**(下記) | **核心** |
...
| 28 | `SpatialFrictionSurvives` | 30日回した後、**同一品目の約定単価の分布が区画によって一致しない**(区画間の価格差が消えていない) | **W2 では、当ててみた変異をどれも落とさない**(下記の囲み)。閾値を置かず「一致しない」ことだけを見る**存在命題**に留まる | |

> **#25 は「`Lines` を逆順に走査する」変異を落とさない(レビュー1・2巡目の実測。全317件が緑のまま)。判別力を持つ資金帯が W2 には存在しない。**
>
> **嗜好(ビール)を1日目に止めているのは手順4 である。** 3巡目に `TradeSystem` へ一時プローブを入れて直接観測し、**1巡目・2巡目に書いた機構の記述(「手順3 で落ちる」「1日目は手順2 で落ちる」)がどちらも誤りだった**ので訂正する。
>
> 観測(seed 1・世帯 Id 0 = Brewer・区画4 = 中心・2日):
>
> | 世界 | day0(1日目) | day1(2日目) |
> | ---- | ------------- | ------------- |
> | 資金100(絞る) | 手順2 通過・**手順3 で落ちる**(実質コスト36 > 予算20) | 手順2 通過・**手順3 で落ちる**(実質コスト42 > 予算0) |
> | 資金1000(対照) | 手順2・手順3 とも通過(予算200 ≥ 36)・**手順4 が0**(`予想在庫 == 目標在庫` で在庫圧力が立たない) | 手順2・手順3 とも通過(予算149 ≥ 42)・**手順4 が1**(`予想在庫` が下がる)→ 約定 |
>
> - **手順2 は day0 から常に通る。** 世帯 Id 0 の区画(中心)は `District.VisionRadius` 内に別の醸造家を直接見通せるので、**記憶を経由しない。**「初日は観測が無いから知っている店が0件」という読みは誤りである
> - **1日だけ回すと、資金をいくら積んでも嗜好は買えない。** 止めているのは手順4(day0 は需要が立たない)であって資金ではない。**だから1日の世界では嗜好側の assert が空振りする**(資金1000倍でも緑のまま)
> - **2日回すと手順4 が開くので、手順3 が実際の分岐点になる。** 絞った世界は手順3 で落ち、対照は通る。**ここで初めて assert が資金の関数になる**
>
> **資金の帯が重ならない理由。** 必需の基礎値は**流動資金**に比率を掛けるが([GDD02 §8.2.7](../03-gdd/02-economy.md) のフォールバック)、嗜好の基礎値は **`SurplusFunds = max(0, 流動資金 − 必要運転資金)`** に掛ける([GDD02 §8.2.1](../03-gdd/02-economy.md))。必要運転資金が大きいため、**嗜好が手順3 を通過できる資金水準では、必需の1日あたりの必要額が無視できるほど小さい。** 逆に必需が資金不足に落ちるまで絞ると、嗜好は手順3 で落ちて手順8 に到達せず、**資金を奪い合わない。**
>
> 探索範囲(seed 1・世帯 Id 0): 資金 50〜1200 を粗く、300〜620 を5刻み、50〜300 を10刻み × 秋/冬 × 1・2・5日目 × 正順/逆順。**`UnaffordableNecessityCount`・嗜好の約定の有無・購入後の流動資金がいずれも1桁まで一致した。** 薪の冬の季節係数 2000‰ は必需側の必要額を約16 → 約33 に動かすだけで、この差を埋めない。
>
> **走査順そのものは実装が守っている**(段5 は `Lines` を並べ替えていない)。**守られていないのは「テストで守られている」という保証のほうである。** 検出器の置き場所(`TradeSystem` の段5 を狭く踏む単体テストを起こすか、#39 が困窮の指標を実際に読むときに併せて立てるか)は本タスクのスコープ外なので **issue へ落とす**。**その担当者は上の観測から出発すること** — **手順4 が開くまでは、資金帯を探しても見つからない。**

> **#28 は「空間の摩擦が消えたら落ちる」テストではない。当ててみた変異をどれも落とさなかった**(レビュー3巡目にフルスイート317件で測り直した。seed 1・30日)。**1・2巡目の表は測った範囲の違う結果を混ぜており、下記3点が誤っていた** — 「移動費を落としても赤になるテストが無い」(実際は #10 が赤)/「#24 が赤」(実際は緑)/「2つ同時だと赤になるテストが無い」(実際は3件が赤)。
>
> | 当てた変異 | #28 | 赤になったテスト |
> | ---------- | --- | ---------------- |
> | 実質コストから移動費を落とす(= `realCost = price`。**実質コストの算出は1か所しかないので、「argmin を提示価格にする」と同一のコード変更である**) | **緑** | **#10 のみ** |
> | 候補集合と移動費を全買い手で共通化する(`isKnown` を常に真 + 距離を固定) | **緑** | #12・#15 |
> | **上の2つを同時に**(買い手の区画が店の選択に一切効かない) | **緑** | #10・#12・#15 |
>
> **W2 で残る価格差の主因は在庫枯渇の順序であって、[GDD06 §2](../03-gdd/06-trade-and-negotiation.md) の空間の摩擦ではない。** 買い手の区画が店の選択から完全に消えても、売り切れの起き方が日ごと・区画ごとに違えば約定単価の集合は一致しない。**したがって #28 が主張できるのは「統合系で区画ごとの約定単価が一致しない」という存在命題までであり、その原因の分解ではない。**
>
> **移動費と実質コストの argmin を守っているのは単体 #10 である。#28 を「空間の摩擦の検出器」として読まないこと。** #38(都市外市場)で候補集合の構造が変われば交絡も変わるので、**そのとき #28 に判別力を持たせられるかを測り直す**(issue へ落とす)。

**テスト #22・#23・#28 は「何日回すか」に依存する。** 日数は仕様ではなく、実装が緑にできる最小の日数でよい。**ただし #25 は例外で、絞った世界を2日以上回すことが規範である**(上記 #25 の行。1日でも緑になるが、そのとき assert が空振りする)。**ただし30日回しても1件も買われないなら、それは実装ミスではなく値の問題(#28)である可能性がある** — その場合は直さずに**止まって報告する**(仕様の穴として数える)。
...
- **段5 の換算経路を通した検証も #38 が引き取る。** 段5 が `TradeSystem.TargetStockInUnits` / `PurchaseQuantityInUnits` を**通っている**ことは、W2 ではどのテストも守っていない(呼び出し側からヘルパー2つを外しても全317件が緑。上記「落ちるべき条件」#6 の囲み)。**ヘルパーの内側は #6 が守っているので、#38 が足すのは「段5 を通したときに耐久の移動費が 1/N になっていない」ことの確認である**(耐久の約定が起きて初めて観測できる)
