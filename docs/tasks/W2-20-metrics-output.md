...
| 1' | `MetricsDoesNotChangeTheStateHash`(**上の #1 の核心変異を差し替える**) | 検証内容は変えない | **核心**。当てる変異を `MetricsSystem.Step` の中の `household.ProductionRuns = 0;`(日次カウンタを読んだついでに消す、という最も起きやすい形)に差し替える / **期待: 赤**。~~`household.UnmetConsumption[0] = 0;`~~ は **no-op なので使えない** — `Item.Grain = 0` で、`WorldDefinition.M0` の `dailyConsumptionPerNpcByRank` は3階層とも穀物が 0、`ConsumptionSystem` が毎日 `required − consumed` を書くので**穀物の欄は全世帯・全日で恒常 0** である。0 に 0 を代入してもハッシュは動かず、**「順10 が `World` を書き換えても緑だった」という誤った実測が doc に残るところだった**(2巡目 I-b) | 2巡目 I-b |
