...
| - | ------ | -------- | -------------------- | ------------------------- |
| 1 | `MetricsDoesNotChangeTheStateHash` | 同じシードで、`MetricsSystem` を登録した走行と登録しない走行を30日回し、`StateHasher.Compute` が一致する | 順10 が `World` を書き換える(集計の途中で配列へ代入する・在庫や資金に触れる) | **核心**(**当てる変異を2巡目 I-b で差し替えた。別表 #1' を見よ**)。~~`MetricsSystem.Step` の中で `household.UnmetConsumption[0] = 0;` を1行足す~~ / **期待: 赤** |
...
| 4 | `InputBlockedCountsTheFirstDay` | 初日(相場基準が無く、必需と耐久が現金を持っていく日)に `input_blocked_households` が 1 以上になる | 段5b の**経路(2)**(資金上限の切り詰め)の計数を落とした。#30 の実測では「入力が最も押し出された日に (b) が ≒0 を返していた」 | **核心**。段5b の経路(2)(`fundsCap == 0`)側の `InputBlockedByFunds` の代入を削る / **期待: 赤(ただし落ちるのは別表 #24 であって、このテスト #4 ではない**。2巡目 I-a。#4 の assert は `>= 1` で、資金 0 の世帯が段4 経路で先に満たしてしまう) |
...
| 21' | `RunIsByteIdenticalAcrossRuns`(**上の #21 を補強**) | 上の #21 に加えて、**(i)** 負号・小数点の表記が異なるカルチャを現在のカルチャに設定した実行の出力が、不変カルチャの実行とバイト一致する、**(ii)** 出力バイトに `\r\n` が現れない | `CultureInfo.InvariantCulture` を落とした / `NewLine = "\n"` を落とした。**同一プロセス内で2回走らせて比べるだけでは、カルチャも改行も列挙順も同じなので3モードとも素通りする**(1巡目 I-a)。`Runner` は `DeterminismConventionTests` の守備範囲外なので、ここが `Runner` の整形規約を守る唯一の機械である。**(ii) は Ubuntu では原理的に赤にならない**(既定の改行が `\n` なので `NewLine = "\n"` を落としても出力が変わらない)。守っているのは開発機(Windows)で走らせたときだけである — 2巡目の「疑い」で確認し、**これ以上は強められないと判断した**(Linux に `\r\n` を既定で吐かせる手段が無い) | 1巡目 I-a / 2巡目 |
| 1' | `MetricsDoesNotChangeTheStateHash`(**上の #1 の核心変異を差し替える**) | 検証内容は変えない | **核心**。当てる変異を `MetricsSystem.Step` の中の **`household.LiquidFunds = 0;`** に差し替える / **期待: 赤**。上の #1 の「この実装ミスで落ちる」欄が名指している「**資金に触れる**」がこれである。**日次カウンタ型の欄は当てても意味が無い** — 下記 | 2巡目 I-b / 3巡目 I-a |
| | **#1 に構造的に残る穴**(3巡目 I-a) | | **順10 が「日次カウンタを読んだついでに消す」型の書き換えをしても、#1 は最終日ぶんしか観測できない。** `ProductionRuns` / `UnaffordableNecessityCount` / `UnmetConsumption` はいずれも**翌日の上流(順1 `ProductionSystem` / 順5 / `ConsumptionSystem`)が当日ぶんを無条件に書き直す**ので、走行中の書き換えは上書きされて消え、30日走行の等値検査に残るのは day 29 の値だけである。[#41 の実測](https://github.com/stama72/visionary/issues/41)では seed 1 の生産は day 23 以降ほぼ 0 なので、`ProductionRuns = 0` は**差し替え先としても no-op になりうる**。初稿の `UnmetConsumption[0] = 0` は恒常 0 で確実に no-op だった(2巡目 I-b)。**`LiquidFunds` を選ぶのは累積量だからである** — 即日分岐して以後の全日に波及するので、最終日の値に依存しない。**この穴は塞いでいない**(順10 が最終日以外に日次カウンタを消す変異を、#1 は検出しない) | |
| 10' | `SettledMedianIsRoundedUpOnEvenCounts`(**上の #10 の入力を直す**) | 検証内容は変えない。**2件の約定の `quantity` を別々の値にする**(たとえば単価 10 を `quantity: 5`、単価 13 を `quantity: 1`)。行単位の中央値なら 12 のまま、数量で重み付けすると 10 側へ寄る | 表 #10 が名指しする3モードのうち「**数量で重み付けした**」は、**2件とも `quantity: 1` だと重み付き中央値と非重み付き中央値が一致するので検出できない**(2巡目 I-a) | 2巡目 I-a |
| 25 | `PartnerSwitchReflectsChangedSellers` | **2日以上**走らせ、前日と当日で同じ品目を**別の**売り手から買った (世帯, 品目) がある日に `partner_switch_permille` が定義された値(同じ相手なら `0`、全部変わったなら `1000`)を返す | **`CurrentCounterpartyId` の代入(段5b)を削った / `BeginDay()` の `Current → Previous` の移し替えを落とした。** どちらも全日 `partner_switch_permille = -1` になるが、**この列を読むテストは #17 の「分母 0 なら `-1`」の枝しか無いため、恒常 `-1` の壊れ方が正常と区別できない。** 規則7 の表が挙げた7行のうち `BeginDay()` と `CurrentCounterpartyId` の2行に落ちるべき条件が無かった(2巡目 I-b)。[TDD01 §5.2](../04-tdd/01-sim-core-and-m0.md) の硬直の判定がこの列を読む | **核心**。段5b の `CurrentCounterpartyId` の代入を削る / **期待: 赤** |
| 26 | `HhiReflectsInternalSettlementConcentration` | 都市内の約定を2人以上の売り手ぶん立てた日に `hhi_permille_squared` が定義された値(手計算した二乗和)を返し、**窓口を相手にした約定がそこに入らない** | 二乗和の取り違え / 窓口を母数に入れた / `CeilDiv` を切り下げにした。**#18 は分母 0 の `-1` の枝しか見ておらず、式そのものを見るテストが1本も無い**(2巡目 I-b) | 2巡目 I-b |
| 27 | `InputBlockedDoesNotLatchAcrossDays` | ある日に `input_blocked` が立った世帯が、**翌日その条件を満たさないなら 0 に戻る**(2日以上走らせる) | **`TradeSystem.Step` 先頭の `BeginDay()` を削った。** `InputBlockedByFunds` は `= 1` しか書かれないので、**一度立った世帯が永久に立ちっぱなしになり `input_blocked_households` が単調増加する。** この列を読む #4 / #5 / #24 / #16 は**すべて1日しか走らせない**ので latch が見えない(2巡目 I-b) | **核心**。`TradeSystem.Step` 先頭の `world.Metrics.BeginDay();` を削る / **期待: 赤** |
| 26' | `HhiReflectsInternalSettlementConcentration`(**上の #26 の入力を直す**) | 検証内容は変えない。**窓口の行を `Sale` かつ相手 = 予約 Id(= 輸出)で足す** | 別表 #26 が名指した「**窓口を母数に入れた**」は、**テストが置いた窓口の行が `Purchase` だったため検出できない** — HHI の母数は `Sale` 行から積むので、`Purchase` 行は変異の有無にかかわらず分子にも分母にも到達しない。**suite 全体で HHI の窓口除外を見る機械が1つも無かった**(3巡目 I-a)。M0 では輸出が都市内約定より桁で大きくなりうるので、除外が落ちると `hhi_permille_squared` が輸出量の分布をそのまま出す | 3巡目 I-a |
| 29 | `TradesAndCountColumnsAreNotSwapped` | どのテストからも1度も読まれていない欄に assert を足す。**最低限**: `active_seller_count` / `active_buyer_count`(`Sale` と `Purchase` の向き)、`internal_settlement_count` / `window_settlement_count` / `internal_settlement_value`(内数と外数)、`settled_min` / `settled_max`(最小と最大の入れ替え)、`export_quantity` / `import_quantity`(`*_value` しか読まれていない) | 向きの反転・内数と外数の入れ替え・最小最大の入れ替え。**どれも値域が自然な非負整数のままなので CSV を読んでも表に出ない。** 2巡目 I-2 と同じ型だが、あちらは収集側の配線、こちらは**出力の列**である(3巡目 I-a)。`active_*` は [GDD02 §8-4](../03-gdd/02-economy.md) の担い手の数、`internal_settlement_*` は都市内取引の消滅([#41](https://github.com/stama72/visionary/issues/41) の主題そのもの)を読む列である | 3巡目 I-a |
| 30 | `CsvHeadersMatchTheValues` | 1日ぶんの既知の `DailySnapshot` を `CsvMetricsSink` に流し、**ヘッダ行を `,` で割った列名と値の対応を名前で引いて**検査する。あわせて5ファイルすべてで**ヘッダのフィールド数とデータ行のフィールド数が一致する**ことを見る | **`Format(...)` の並びを2つ入れ替える / 1つ落とす変異が、テストもビルドも `dotnet format` も緑で通る。** ヘッダは手書きの文字列、値は手書きの配列で、**両者の対応を見る機械が75列ぶん1つも無い**(3巡目 I-a)。既存の #21(同じバイナリを2回)・#21'(カルチャと改行)・#22(**行数だけ**。1行あたりのフィールド数もヘッダとの対応も見ていない)はどれも並びに対して不変である。`necessity_blocked_households` と `necessity_blocked_count`、`export_value` と `export_quantity` のように**同じ次元で入れ替わっても不自然に見えない組**が複数ある。列名で読む下流([#209](https://github.com/stama72/visionary/issues/209) の判定関数と、開発者の目視)が別の量を名前どおりに信じる | 3巡目 I-a |
| 28 | `SellerWithoutReferenceIsDistinctFromNoSettlement` | **2日目以降**を見て、`SellerHasNoReference` が「相場基準が立たない売り手日」であり「**前日の約定が無い**売り手日」ではないことを分ける(別表 #8' が既に作っている「初日は基準なし・2日目は基準あり」の世界に `seller_days_without_reference` の assert を足せばよい: day0 = 1、day1 = 0) | 段1 の配線を `SellerHasNoReference[id] = hasSettled ? 0 : 1;` に取り違えた。**表 #7 は初日1日だけを見ており、初日は全売り手について `hasReference == hasSettled == false` と縮退するので、この取り違えが緑のまま通る**(2巡目 I-a)。取り違えると [GDD02c §1.2](../03-gdd/02c-price-and-budget.md) の「盲目」の切り分けが静かに無意味になる | 2巡目 I-a |
