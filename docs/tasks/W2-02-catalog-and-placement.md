...
**すべて初期値であり調整対象**(CLAUDE.md「数値(閾値・係数)は初期値であり調整対象。固定すべきは構造と依存関係」)。**検算と調整は [#28](https://github.com/stama72/visionary/issues/28)。** 出典を持つのは**レシピの品目の組**([GDD02 §2.4](../03-gdd/02-economy.md))と、**1次産品(品目 0〜3)の取得原価**([GDD02 §10.2](../03-gdd/02-economy.md))である。**数量・所要労働‰・都市生産品(品目 4〜8)の金額はここが初出である**([GDD02 §13.2](../03-gdd/02-economy.md) が「値として残っているもの」に挙げている項目そのもの)。

> **1次産品の取得原価は初出ではない。上流が固定している。**([1巡目レビュー I-b-1 の訂正](#1巡目レビューで足したテスト別表))
>
> [GDD02 §10.2](../03-gdd/02-economy.md) は外部売値を **穀物 10 / 木材 8 / 鉄鉱石 14 / 木炭 12** と決定しており、同節が「**1次産品の仕入単価は外部売値に固定される**」「**外部売値が1次産品の価格の天井になる**」と書いている。1次産品は都市内に売り手が居ないので、仕入れ移動平均単価の初期値は外部売値そのものでなければならない。**当初この表は鉄鉱石を 20 と書いていたが、天井を超える値であり誤りだった。** 調整の余地があるのは都市生産品(4〜8)だけであり、そちらは §10.2 の不等式 `原価 < 外部買値 < 原価下限` の検算とともに [#28](https://github.com/stama72/visionary/issues/28) が持つ。
...
| ---- | -- | -- | -- | -- | -- | -- | -- | -- | -- |
| `InitialAcquisitionCost` | 10 | 8 | 14 | 12 | 22 | 6 | 16 | 30 | 60 |
...
| 3  | `PlacementIsDeterministicForTheSameSeed` | 同一シードから2回生成した `World` の `StateHasher.Compute` が一致する(3シード) | **生成に乱数源の外の入力を混ぜる**(`DateTime.Now` / `Guid.NewGuid` / シード無しの `Random`)/ **`WorldDefinition.M0` を `static` な可変テーブルにして生成のたびに書き換える**(1回目と2回目で土台が変わる)。**系統借用と列挙順はこのテストでは捕まらない** — 下記「乱数の系統について」のとおり |  |
...
**同じ理由で「列挙順が不定のコレクションを使っていないこと」も #3 では押さえられない。** `Dictionary` に載せても、**同一プロセス・同一挿入順なら列挙順は再現する**ので #3 は緑のまま通る。ADR-0002 がこれを禁じているのは実行環境やバージョンをまたいだときに崩れるからであって、同一プロセス内の2回の比較では原理的に差が出ない。**ここもレビュー観点として残る**(1巡目レビュー I-b-3。当初この表は両方を #3 の変異欄に書いていたが、いずれも落ちない)。

## 1巡目レビューで足したテスト(別表)

**上の表は implementer に渡した時点の指示であり、最終形ではない。** 1巡目のレビューが象限I-b(仕様そのものの欠陥)として2件を挙げたので、ここに足す。**上の表を書き換えずに別表にするのは、「何を最初に指示し、何を後から足したか」を残すためである。**

**足す理由は共通して「生成順序が書く欄に、対応するテストが無い」ことである。** 仕様の生成順序(上記)は世帯に7種の値を書くが、当初の表が読んでいたのは `Occupation` / `WorkshopInventory` / `PurchaseUnitCostAverage` / `Rank` / `MemberNpcIds` だけで、**`LiquidFunds`・`HouseholdInventory`・`SkillPermille` は1本も読まれていなかった。** これらは**緑のまま壊せる** — しかも「数値は初期値であり調整対象」の枠に入るので、#34 以降で挙動が変でも**配線ミスではなく調整不足として処理され、誤配線を疑う契機が構造的に生まれない**。

| #  | テスト | 検証内容 | この実装ミスで落ちる | 核心 |
| -- | ------ | -------- | -------------------- | ---- |
| 22 | `EveryHouseholdStartsWithTheDefinedLiquidFunds` | 全世帯の `LiquidFunds` が `definition.InitialLiquidFunds` と一致 | 資金を配り忘れて 0 のままにする / 定義ではない定数を書く。**初日から全世帯が [GDD02 §6.3](../03-gdd/02-economy.md) の予算制約に張り付き、#34 の取引が一度も成立しない** |  |
| 23 | `HouseholdInventoryIsSeededFromTheDefinition` | 全世帯・全品目で `HouseholdInventory[i] == definition.InitialHouseholdInventory[i]` | **世帯在庫と工房在庫を取り違える** / 0 のままにする / `InitialAcquisitionCost` を書き込む(**長さが同じ `int[]` なので型では止まらない**)。`M0` は薪4・パン4・ビール2 を持つので 0 との差が出る。**[GDD02 §8.1](../03-gdd/02-economy.md) の必需品の初期在庫が消え、#34 の消費が初日に欠乏を起こす** |  |
| 24 | `SkillPermilleFollowsTheNpcRank` | 全 NPC について、`Rank` が `Master` なら `SkillPermille == InitialSkillPermilleByRank[(int)NpcRank.Master]`、`Apprentice` なら同様に `Apprentice` の値。かつ**両者が異なる値である**こと | **`WorldGenerator` が階層の添字を取り違える**(親方に徒弟の値が届く)。[TDD01 §3.2](../04-tdd/01-sim-core-and-m0.md) が「**器として先に持つ**」と決めた熟練度の、階層への配線が入れ替わる。上記の注記が「全員同じ値だとハッシュから落としても気付けない」として別値を置いた当の理由が、**その別値が正しい階層に届いているかを誰も見ていない**ままだった。**`M0` の表そのものの転置は捕まえない**(下記) |  |
| 25 | `PrimaryGoodCostsMatchTheExternalSellPrices` | `M0.InitialAcquisitionCost` の品目 0〜3 が [GDD02 §10.2](../03-gdd/02-economy.md) の外部売値 **10 / 8 / 14 / 12** と一致する | 上流の決定を写し間違える(**本タスクが実際に鉄鉱石を 20 と書いた誤りそのもの**)。§10.2 は「1次産品の仕入単価は外部売値に固定される」「外部売値が1次産品の価格の天井になる」と決定しており、**天井を超える取得原価は都市内の誰も支払えない。** #28 が調整してよいのは都市生産品(4〜8)だけである |  |

- **核心印は付けない。** 4件とも単一の値の表明であり、上の「核心印について」の基準(テストが自分で中間の集合を組み立てるか)に当たらない。**#12 と同じく全世帯を走査するが、走査の対象は定義から直接引ける値であって、テスト側が集合を構築するわけではない**
> **熟練度‰ は [GDD02 §5.2](../03-gdd/02-economy.md) の労働力係数‰ ではない。混同しないこと。**(2巡目レビュー I-b の訂正)
>
> **当初この行は「§5.2 の労働力が階層で逆転する」と書いていたが、誤りである。** §5.2 の労働力係数‰(親方 1000 / 徒弟 300、M0 の世帯合計 1300‰ 固定)は**階層ごとの定数表であって、本タスクはどこにも持っていない**(`WorldDefinition` に労働力係数の欄は無い)。`SkillPermille` を反転しても M0 の労働力は一切動かない — [TDD01 §3.2](../04-tdd/01-sim-core-and-m0.md)「**熟練度は持つ。ただし M0 では読まない**」、[GDD08 §9](../03-gdd/08-household-and-decision.md)「熟練度・時間配分・家事 → **入れない**」「§4.2 の生産性式は**この式自体を使わない**」。
>
> **単位(‰)も形(階層別の表)も §5.2 と同型なので、取り違えたまま #34 へ渡ると実害が出る。** #34 が「階層で係数が違う」配線を探して `SkillPermille` を見つけ、`Σ SkillPermille = 800‰` を労働力として使うと、**正しい 1300‰ より全職業の生産量が静かに下がったまま M0 の全実験が回る**(生産は「遅いが動く」ので赤にならない)。同時に、TDD01 §3.2 が置いた歯止め(「**読む処理を足すときは §9 の M0 スコープ表を先に直すこと**」)も「もう §5.2 の機構だ」と読めるので発動しない。**この行と、対応するテストの doc コメントに、§5.2 の労働力係数は別物であり #34 が持つことを書き分ける。**

> **#24 は `M0` の熟練度表の転置を捕まえない。狭く読むこと。**
>
> `{ 700, 400, 100 }` を転置しても、**テストの期待値も実装も同じ `definition.InitialSkillPermilleByRank` を読むので緑のまま通る。** `Assert.NotEqual` が見ているのは「両者が異なること」だけで、親方 > 徒弟 の向きは誰も押さえていない。**#24 が押さえるのは `WorldGenerator` の配線だけである。**
>
> **#25 のように `M0` の値を直接固定するテストを足さないのは、熟練度‰ に上流の照合先が無いためである。** 取得原価は [GDD02 §10.2](../03-gdd/02-economy.md) が値を決定しているので #25 が写し間違いを捕まえられるが、熟練度‰ は「初期値であり調整対象」であり、向きすら GDD が要求していない(上記の注記のとおり、別値にした理由は**ハッシュの回帰を鈍らせないため**であって階層の序列を表現するためではない)。**要求されていない不変条件を先回りして固定すると、#28 の調整と衝突する。** CLAUDE.md「迷ったら狭い側に倒す」に従い、**捕まえないことを書いて残す**([docs/process/02](../process/02-task-spec.md) 規則4「保証を書くときは残る穴も書く」)

- **#25 を `WorldDefinition` の検証(コンストラクタ)にしない。** 1次産品が 0〜3 であることは `Item` の採番の知識であり、`WorldDefinition` は品目の意味を知らない(`ItemCount` しか持たない)。**コンストラクタに持ち込むと、定義が品目の意味に依存し、#28 や JSON 設定([TDD01 §4.1](../04-tdd/01-sim-core-and-m0.md))で品目を足すときに崩れる。** `M0` の表の写し間違いを捕まえるのが目的なので、`M0` を見るテストが正しい置き場所である
...
- [ ] 「落ちるべき条件」のテスト(#1〜#21)と「1巡目レビューで足したテスト」(#22〜#25)が全て緑
