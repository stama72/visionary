...
| 24 | `SkillPermilleFollowsTheNpcRank` | 全 NPC について、`Rank` が `Master` なら `SkillPermille == InitialSkillPermilleByRank[(int)NpcRank.Master]`、`Apprentice` なら同様に `Apprentice` の値。かつ**両者が異なる値である**こと | **階層の添字を取り違える**(親方100‰・徒弟700‰ になる)/ 全員に同じ値を配る。[GDD02 §5.2](../03-gdd/02-economy.md) の労働力が階層で逆転し、**§5.2 の「階層で係数が違うこと」を M0 で見たいという目的が反転したまま通る。** 上記の注記が「全員同じ値だとハッシュから落としても気付けない」として別値を置いた当の理由が、**その別値が正しい階層に届いているかを誰も見ていない**ままだった |  |
