...
public sealed class M0CalibrationTests
{
    ...
    /// </para>
    /// <para>
    /// <b>M9(<c>BuildM0</c> の鍛冶を <c>laborPermille: 1000</c> に戻す変異)はこの書き換え後の
    /// テストで赤になる見込み(タスク仕様。年間実行回数156、収支+8,520)。書き換える<b>前</b>の
    /// テスト(日ごとの floor × 120、旧版)には当てても緑である(#218 決定ログ5 の訂正)。
    /// mutatorの実測はレビューの巡が閉じた後にまとめて渡される(結果はこの remarks へ転記する)。
    /// </para>
    /// </remarks>
    [Fact]
