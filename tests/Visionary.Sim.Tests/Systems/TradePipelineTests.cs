...
public sealed class TradePipelineTests
{
    ...
    private sealed class CitySurvivalScan
    {
        ...
        /// <summary>
        /// #174 テスト表 #7。自分の出力品目(目標日数が正のものに限る)の世帯在庫が0の日に
        /// 工房在庫が正だった(世帯Id, 品目Id, 日)。
        /// </summary>
        /// <remarks>
    ...
    /// <b>M-8の現在の赤/緑はまだ実測していない</b>(このあと<c>mutator</c>が実測する)。
    /// 構造的な留め具は本タスクでは足さない ── それは#173の持ち物であり、フェーズ3がissueへ
    /// 切り出す予定である。条件1・条件2は#174 の後も反転側のみのままであり、上で述べた
    /// 「数え落とし」方向の非対称はそちらには残っている(向きが反転しきって条件1・2にも正側が
    /// 立てば、それについては解消する)。
    /// <para>
