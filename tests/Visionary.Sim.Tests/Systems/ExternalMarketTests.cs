...
public sealed class ExternalMarketTests
{
    ...
    /// <c>39e367a</c>、M-1)。</b><c>clamp</c> の上限を 2000 → 1000 にする変異は赤になった。
    /// ただし赤くなったのは<c>mr:14</c>/<c>mr:20</c>/<c>mr:30</c>の3点だけで、
    /// <c>mr:6</c>/<c>mr:7</c>/<c>mr:10</c>は緑のままだった ── 上限側の点だけが上限
    /// 2000を守っている。
    /// </remarks>
    /// <remarks>
    ...
    /// <remarks>
    /// <b>変異の実測(2026-09-21、<c>mutator</c> が使い捨てworktreeで測定、対象コミット
    /// <c>39e367a</c>、M-10)。</b>係数‰の<c>CeilDiv</c>を<c>FloorDiv</c>にする変異は赤に
    /// なったが、赤くなったのは<c>mr:9</c>の1点だけで、他の6点(6/7/10/14/20/30)は緑の
    /// ままだった(上のR-4の予測どおり)。
    /// </remarks>
    [Theory]
    [InlineData(6, 0)]
