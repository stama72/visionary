...
- **開発者の判断を要するタスクの WIP は 1**(設計・ADR・ゲームデザイン)。仕様が凍結済みの実装タスクに限り 2 本まで。**パイプラインの無人フェーズは開発者を消費しないので、この数に入らない** — ただし**走っている間、本体の `visionary/` はパイプラインのものである。** 並行して進めるタスクは worktree を分ける([05-phase-sessions](docs/process/05-phase-sessions.md))
...
**パイプラインは停止則4つ**(`SPEC-OUTSIDE` / `IMPL-BLOCKED` / `REVIEW-EXHAUSTED` / `RED`)**に当たったときだけ止まり、デスクトップ通知を出す。** `SPEC-OUTSIDE`(フェーズ2 が GDD / TDD / ADR を触った)は機械が差分で見ている。止まったら `.pipeline/*.jsonl`(走行中も伸びる生ログ)を読む。手で回すときは `/clear` してから `/impl <issue番号>` `/wrap <issue番号>` を開く。
