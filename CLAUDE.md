...
**スクリプトを打つのはフェーズ1 のセッションである。** 仕様を凍結したら自分で背景起動して止まる。開発者に打たせると「終わったので次を打つ」待ちが残り、ADR-0010 が消しにきたものがそのまま残る。**待っている間にフェーズ1 が使うターンは 0 である**(W2-04 実測: 36分・0ターン)。**打つ前に 5 時間枠の残りを読み、40% 未満なら打たない**(読み方は [05](docs/process/05-phase-sessions.md)、規則は [04「枠」](docs/process/04-issue-driven.md))。
