...
**どのマシンで開くかは [05「どこで走らせるか」](docs/process/05-phase-sessions.md) が持つ。** 実装タスクはフェーズ1 からデスクトップ、`/design` はラップトップである。**デスクトップのフェーズ1 は `claude --bg` で生やして `claude attach` で繋ぐ** — 素の `claude` で開くと、端末が切れたときに**停止で起こされる先と `DONE` 時の独立確認**が消える(停止通知は `pipeline.ps1` が出すので、**それが道連れで落ちなければ**届く。落ちるかは未測)。**フェーズ1 と `pipeline.ps1` の間にマシン境界を入れない** — 背景起動が ADR-0010 の要だからである。
