...
**スクリプトを打つのはフェーズ1 のセッションである。** 仕様を凍結したら自分で背景起動して止まる。開発者に打たせると「終わったので次を打つ」待ちが残り、ADR-0010 が消しにきたものがそのまま残る。**待っている間にフェーズ1 が使うターンは 0 である**(W2-04 実測: 36分・0ターン)。**5 時間枠の残りは `pipeline.ps1` 自身が起動時に読み、40% 未満なら起動を拒否する**(終了コード 4。拒否されたらリセット時刻を開発者に報告して止まる。規則は [04「枠」](docs/process/04-issue-driven.md)、機械の範囲は [05](docs/process/05-phase-sessions.md))。**本体が `origin/master` より遅れていても拒否する**(終了コード 5。ラップトップで凍らせた仕様を pull し忘れると、一世代前の仕様を正確に実装した結果が残り、読んでも正しく見える)。
