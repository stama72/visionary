...
**どのマシンで開くかは [05「どこで走らせるか」](docs/process/05-phase-sessions.md) が持つ。** 実装タスクはフェーズ1 からデスクトップ、`/design` はラップトップである。**デスクトップのフェーズ1 は `claude --bg` で生やして `claude attach` で繋ぐ** — 素の `claude` で開くと、端末が切れたときに**停止後の自動再開と `DONE` 時の事後検証**が消える(停止通知自体は `pipeline.ps1` が出すので届く)。**フェーズ1 と `pipeline.ps1` の間にマシン境界を入れない** — 背景起動が ADR-0010 の要だからである。
