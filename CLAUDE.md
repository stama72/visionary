...
- **ADRか他文書かの切り分けは3つの問いで判定する**([docs/adr/README.md](docs/adr/README.md) 運用ルール):
  1. **却下した選択肢に価値があるか。** 後から覆す人が選択肢の再発見からやり直す必要があるならADR。「他に選びようがなかった」ものは単なる仕様
  2. **これは決定か、予測か。** 予測は運用で反証されるのが正常。凍る文書に予測を書くと、外れたときにADRを起こすしかなくなる
  3. **読者は「なぜ」を知りたいのか、「今どうなっているか」を知りたいのか。** 現行の状態を答える責務を負った時点で、その文書はADRではない
- **開発プロセスの現行仕様は [docs/process/](docs/process/)** が持つ。運用のフィードバックはまずそこと `.claude/` に反映し、**ADRを起こすのは過去のADRの決定を覆すときだけ**にする
