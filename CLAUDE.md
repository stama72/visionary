...
**どのマシンで開くかは [05「どこで走らせるか」](docs/process/05-phase-sessions.md) が持つ。** 実装タスクはフェーズ1 からデスクトップ、`/design` はラップトップである。**デスクトップのフェーズ1 は `claude --bg` で生やして `claude attach` で繋ぐ** — 素の `claude` で開くと、端末が切れたときに停止の受け取り先ごと消える。**フェーズ1 と `pipeline.ps1` の間にマシン境界を入れない** — 背景起動が ADR-0010 の要だからである。
