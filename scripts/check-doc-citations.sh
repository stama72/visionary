...
#    枝番引用は4件: `WorldGeneratorTests.cs:55` の §8-4、`ObservationsTests.cs:46`
#    の §8-7、`StateHasher.cs:116` と `StateHasherTests.cs:104` の TDD01 §3.3-9。
#    検証項目表やパイプライン順表の行が入れ替わると、この4件は別の項目を指す
#    誤った引用になるのに、この検出器は 0 件を返し続ける。1.(番号の再利用)
#    とは別物 — 1. は「番号が存在してしまう」case、こちらは「そもそも読んで
#    いない」case。
#
# 6. 見出しの収集側が過剰に採る場合。1.〜5. はすべて引用側の限界だが、これは
#    照合相手の集合そのものの限界。見出しの収集はコードフェンスを見ないので、
#    GDD/TDD のフェンス内に "## 3. ..." のような行が入ると幻の節ができ、
#    存在しない節への引用が 0 件のまま通る(2026-09-20 実測で該当ゼロ。潜在的な
#    穴)。0 件は「引用先が実在する」ではなく「引用先が ^#{2,6} <番号> で始まる
#    行として存在する」しか意味しない。
#
# --- "0 件" が保証する範囲(2026-09-20 実測で確定) --------------------------
#
# 保証するのは、同一行に "<GDD|TDD>dd[a-z]* §<番号>" の形で書かれた 657 件の
# (文書Id, 節番号) の組が、現行の GDD / TDD の見出しとして存在することだけ。
# 保証しないもの: 節が引用の主題と一致すること(3.) / 番号が再利用されていない
# こと(1.) / 枝番の指す行(5.) / UNRESOLVED の 35 件(2.) / 見出し集合の正しさ
