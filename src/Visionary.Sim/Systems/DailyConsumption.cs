...
/// <summary>
/// 世帯の1日消費量(GDD02b §1 / GDD02d §5)。<see cref="ConsumptionSystem"/> の私有メソッドを
/// 切り出したものであり、複製ではない。
/// </summary>
/// <remarks>
/// <b>置き場所は1か所にしか無い。</b>GDD02b §2 の目標在庫(必需・嗜好)は「今後 N 日に
...
{
    /// <summary>世帯の1日消費量(GDD02b §1 / GDD02d §5)。</summary>
        ...
        WorldDefinition definition, World world, HouseholdState household, int itemId, Season season)
    {
        ...
        foreach (int npcId in household.MemberNpcIds)
        {
            ...
            // 季節係数を掛けるのは薪だけ(GDD02d §5)。構成員ごとに切り上げてから合計する ──
            // 世帯合計に先に掛けると、基礎量が奇数の場合に結果がずれる(GDD02b §1)。
    ...
    /// <summary>
    /// 今日から <paramref name="days"/> 日ぶんの消費量の合計(GDD02b §2「N日分は先読みで数える」)。
    /// </summary>
    /// <remarks>
    /// <b>日ごとに季節係数を適用して切り上げてから足す。</b>合計してから切り上げない
    /// (GDD02b §2)。<paramref name="now"/> の時刻(<see cref="Tick.HourOfDay"/>)は捨てる ──
    /// 「今日」は日単位の概念である。
    /// </remarks>
