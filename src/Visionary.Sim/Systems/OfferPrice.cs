...
public static class OfferPrice
{
        ...
        int floorPrice, int marketReference, int sellableStock, int shipmentTargetStock, int isBankrupt)
    {
        ...
        // 破産中の枝ではStockRatioPermilleを呼ばない。在庫比を評価してから係数を捨てる書き方だと
        // 「出荷目標在庫 <= 0」の検査が破産中だけ効くという非対称が入る。
        int coefficientPermille = isBankrupt == 1
            ? BankruptCoefficientPermille
            : PriceCoefficientPermille(StockRatioPermille(sellableStock, shipmentTargetStock));
